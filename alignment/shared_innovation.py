...
"""
polynesian_shared_innovations.py
...
Copied from alignment/ in sigmorphon-2023 repo (Ting Chen, Annie Zhang)
...
def get_innovations_with_little_environment(daughter, protoform):
    ...
    daughter, protoform = daughter.strip(), protoform.strip() # remove wierd spaces
    innovations = set()
    ...
    #alignment = needleman_wunsch(daughter, protoform)[0]
    ...
    #aligned_daughter, aligned_proto = trim_partial_cognate(aligned_daughter, aligned_proto)
    

    # if daughter_phoneme == "-": deletion
    # elif proto_phoneme == "-": insertion
    # else: substitution
...
def run(input, PROTOLANG):
            ...
            for daughter in daughters:
                ...
                if row[daughter] != "-" and row[daughter] != "?":
                    ...
                    for innovation in innovations:
                        ...
                        # if daughter not in language_to_rules:
                        #     language_to_rules[daughter] = [inno]
                        # else:
                        #     print(f"repeating {inno}")
                        #     language_to_rules[daughter].append(inno)
                                
                        # if inno not in rules_to_language:
                        #     rules_to_language[inno] = [daughter]
                        # else:
                        #     rules_to_language[inno].append(daughter)
    ...
    for daughter in daughters:
        ...
        for key in shared_innovations:
            if daughter in shared_innovations[key]:
                ...
                line += "0"
        lines.append(line+"\n")
    # with open("output/temp_ipa_segs_4.17", "w") as f:
    #     f.writelines(lines)
    ...
    for align in alignments:
        ...
        lines.append(line)
    # with open(f"output/alignment_VtoC_penalty_{syl}{cons}", "w", encoding="utf-8") as f:
    #     f.writelines(lines)
    with open(f"output/alignment_Prototucanoan", "w", encoding="utf-8") as f:
        f.writelines(lines)
    # print(f"({syl},{cons}): misalignment {total_misalignment}, insdel {total_insdel}")
    print(f"misalignment {total_misalignment}, insdel {total_insdel}")
        
    # file = open('output/alignment_VtoC_penalty_10_p', 'wb')
    # pickle.dump(pickleline, file)

    # json_object = json.dumps(language_to_rules, indent=4)
    # with open("output/language_to_rules.json", "w") as outfile:
    #     outfile.write(json_object)

    # json_object = json.dumps(rules_to_language, indent=4)
    # with open("output/rules_to_language.json", "w") as outfile:
    #     outfile.write(json_object)
    
    # json_object = json.dumps(sound_change_table, indent=4)
    # with open("output/language_table.json", "w") as outfile:
    #      outfile.write(json_object)
...
def compare(file_1, file_2):
    ...
    #     if (line1 != line2):
    #         # print(f"%s\n%s\n\n", line1, line2)
    #         d1 = line1.split(">")[0].strip()
    #         d2 = line2.split(">")[0].strip()
    #         p1 = line1.split(">")[1].split("/")[0].strip()
    #         p2 = line2.split(">")[1].split("/")[0].strip()
    #         out.write(d1)
    #         out.write("    ")
    #         out.write(d2)
    #         out.write("\n")
...
if __name__ == "__main__":
    input = "data/polynesian-new.cog"
    PROTOLANG = 'ProtoPolynesian'
    input2 = "E:/Documents/GitHub/sigmorphon-2023/tuknoan-preprocessing/temp.csv"
    ...
    # print(feature_edit_distance.dst.fm.weights)
    # file1 = "output/alignment_VtoC_penalty_custom_match7"
    # file2 = "output/alignment_VtoC_penalty_custom_match8"
    file1 = r"E:\Documents\GitHub\sigmorphon-2023\alignment\output\alignment_Prototucanoan"
    file2 = r"E:\Documents\GitHub\sigmorphon-2023\tuknoan-preprocessing\expert_alignment.csv"
    compare(file1, file2)
